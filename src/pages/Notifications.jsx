...
import { notifications as demoNotifs } from '../data/demoData';
...
export default function Notifications() {
  ...
  const aiNotifs = [
    ...
    { id: 'ai2', type: 'scheme_eligible', title_en: 'PM Vishwakarma Toolkit Grant', title_hi: 'PM विश्वकर्मा Toolkit Grant', body_en: 'Ready for ₹15,000 toolkit grant. Upload Aadhaar.', body_hi: '₹15,000 toolkit grant तैयार। Aadhaar अपलोड करें।', is_read: false, created_at: new Date(Date.now() - 7200000).toISOString() },
    { id: 'ai3', type: 'doc_pending', title_en: 'Upload PAN Card', title_hi: 'PAN कार्ड अपलोड करें', body_en: 'PAN card required for MUDRA Kishor tier.', body_hi: 'MUDRA किशोर tier के लिए PAN कार्ड अनिवार्य।', is_read: true, created_at: new Date(Date.now() - 172800000).toISOString() },
  ];

  const mappedDemo = demoNotifs.map(n => ({
    id: n.id, type: n.type, is_read: n.read,
    title_en: n.title, title_hi: n.titleHi || n.title,
    body_en: n.body, body_hi: n.bodyHi || n.body,
    created_at: new Date(Date.now() - Math.random() * 86400000 * 3).toISOString()
  }));

  const allNotifs = [...aiNotifs, ...mappedDemo].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  ...
  const handleMarkRead = (id) => { setReadState(prev => ({ ...prev, [id]: true })); markNotificationRead(); };
    ...
    <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.05 } } }}
      ...
      <motion.div variants={fadeUp} className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          {lang === 'hi' ? 'AI अलर्ट और स्मार्ट सूचनाएं' : 'AI Alerts & Smart Notifications'}
        </p>
      </motion.div>
      ...
      <motion.div variants={fadeUp} className="flex gap-2 overflow-x-auto hide-scrollbar pb-1 mb-5">
        {[
          ...
          { id: 'scheme_eligible', label: '🤖 AI' },
          { id: 'payment_received', label: '💰' },
          { id: 'doc_pending', label: '📄' },
        ].map(f => (
          <button key={f.id} onClick={() => setFilter(f.id)}
            className={`px-4 py-2 rounded-xl text-sm font-semibold whitespace-nowrap transition-all ${
              filter === f.id 
                ? 'bg-gradient-to-r from-[#1F3C88] to-[#4A90E2] text-white shadow-md' 
                : 'bg-white text-slate-500 border border-slate-200 hover:border-[#4A90E2]/30'
            }`}>
            {f.label}
          </button>
        ))}
          ...
          filtered.map(n => {
            ...
            return (
              <motion.div key={n.id} variants={fadeUp}
                className={`bg-white rounded-xl border p-4 flex items-start gap-3 cursor-pointer hover:shadow-md transition-all ${
                  !isRead ? 'border-[#4A90E2]/30 shadow-sm' : 'border-slate-100'
                }`}
                onClick={() => !isRead && handleMarkRead(n.id)}>
                <div className="w-10 h-10 rounded-xl bg-[#EAF4FF] flex items-center justify-center text-base shrink-0">
                  {typeEmojis[n.type] || '🔔'}
                </div>
